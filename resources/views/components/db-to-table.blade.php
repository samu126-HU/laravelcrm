                ...
                <div
                    ...
                    @if ($enableUpdate && isset($row['id']))
                        <form method="GET"
                            ...
                            <button type="submit"
                                class="bg-indigo-700 hover:bg-indigo-500 p-2 rounded-md w-full sm:w-auto">{{ __('Update') }}</button>
                        </form>
                    @endif
                    @if ($enableDelete && isset($row['id']))
                        <form method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="bg-red-700 hover:bg-red-500 p-2 rounded-md w-full sm:w-auto">{{ __('Delete') }}</button>
                        </form>
                    @endif
                    {!! trim($slot ?? '') !!}
                </div>
